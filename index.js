...
app.use(fileUpload({ // Tambahkan middleware file upload
...
const apiConfig = require('./settings.json');
    ...
    apiConfig.categories.forEach(category => {
        category.items.forEach(item => {
            ...
            endpointConfigs[cleanPath] = {
                ...
                path: item.path,
                method: item.method || 'GET' // Tambahkan support method
            };
        });
    });
...
// Modifikasi bagian ini untuk handle GET dan POST
Object.entries(scrapers).forEach(([route, { handler, config }]) => {
    ...
    if (config.method === 'POST') {
        app.post(route, checkApiKey, async (req, res) => {
            try {
                // Untuk endpoint upload gambar, kita kirim req object lengkap
